  ...
  app.get("/api/company/credit",
    ...
    async (req: AuthRequest, res) => {
      try {
        ...
        // Try the native credit endpoint first (only works if credit feature is enabled on the store)
        try {
          const creditResult = await bigcommerce.getCompanyCredit(companyId);
          if (creditResult?.data) {
            Object.assign(creditData, { creditEnabled: true, ...creditResult.data });
            return res.json(creditData);
          }
        } catch {
          // Credit feature not enabled — fall through to extraFields
        }

        // Fall back to extraFields on the company record
        try {
          const companyResult = await bigcommerce.getCompanyDetails(companyId);
          const extraFields: any[] = companyResult?.data?.extraFields || [];
          ...
          // Company details unavailable — return safe defaults
        }
